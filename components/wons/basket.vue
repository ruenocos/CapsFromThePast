  ...
  methods: {
    ...
    removeItem(item) {
      ...
      if (item.quantity === 0) {
        ...
        sessionStorage.setItem(item.key + '--' + item.size, JSON.stringify(item))
      }
    ...
    updateObjectData() {
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        basket_items.push(JSON.parse(sessionStorage.getItem(sessionStorage.key(i))))
      }
...
.basket {
  @apply w-1/2 h-1/2 flex flex-col p-4;
...
.item-props {
  @apply flex flex-col px-4 justify-center h-full;
...
.basket-row-checkout {
  @apply flex self-end justify-between w-full;
}
</style>
